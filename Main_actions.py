...
for f in path_f:
	...
	stepX = stepY+stepY//2 #1,5 размера - это экспериментально для конкретного бланка, так-то в общем виде нельзя
	#todo - продумать object-detection готовых пар (20,40) или (30,60), а также (1,21), (1,31)
...
#################
# а теперь надо убедиться, сколько секций еще имеется на бланке - найти число в 2 раза большее на той же координате Y, начиная с середины бланка
# Все лишь для того, чтобы убедиться, сколько точно секций, чтобы потом вычислять размеры ячеек, где записан ход
# todo надо выносить весь этот бойлерный код в отдельную процедуру
